...
def main():
    ...
    def compute_metrics(preds, labels):
        ...
        result_bleu_l = metric_bleu.compute(predictions=decoded_preds, references=decoded_labels)
        logger.info(f"PREDICTION: {decoded_preds[0]}")
        logger.info(f"GROUNDTRUTH: {decoded_labels[0]}")

        result = metric.compute(predictions=decoded_preds, references=decoded_labels, use_stemmer=True)
        result["bleu"] = result_bleu["bleu"]
        result["bleu_l"] = result_bleu_l["bleu"]
        x = json.dumps(result, indent=3)
        logger.info(f"EVAL METRIC\n {x}")
    ...
    adamw = AdamW(
        lr=linear_decay_lr_schedule_fn,
        eps=training_args.adam_epsilon,
            )
    '''
    # Setup train state
        ...
        if jax.process_index() == 0:
            ...
            if metrics:
                ...
                metrics = {
                    ...
                    for metric_name, value in metrics.items() if "bleu" not in metric_name
                }
